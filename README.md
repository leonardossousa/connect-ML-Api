...
- 🎯 Entry points to execute your program using `python -m <connect_ml_api>` or `$ connect_ml_api` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/leonardossousa/connect-ML-Api/branch/main/graph/badge.svg?token=connect-ML-Api_token_here)](https://codecov.io/gh/leonardossousa/connect-ML-Api)
[![CI](https://github.com/leonardossousa/connect-ML-Api/actions/workflows/main.yml/badge.svg)](https://github.com/leonardossousa/connect-ML-Api/actions/workflows/main.yml)

Awesome connect_ml_api created by leonardossousa

## Install

from source
```bash
git clone https://github.com/leonardossousa/connect-ML-Api connect_ml_api
cd connect_ml_api
make install
```
...
```bash
pip install connect_ml_api
```

## Executing

```bash
$ connect_ml_api run --port 8080
```

or

```bash
python -m connect_ml_api run --port 8080
```

or

```bash
$ uvicorn connect_ml_api:app
```
...
```bash
❯ connect_ml_api --help
Usage: connect_ml_api [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ connect_ml_api create-user --help
Usage: connect_ml_api create-user [OPTIONS] USERNAME PASSWORD
...
```bash
connect_ml_api create-user admin admin --superuser
```
...
Run with `connect_ml_api run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `connect_ml_api create-user`.
...
-----------------------------------------------------
connect_ml_api/__init__.py              4      0   100%
connect_ml_api/app.py                  16      1    94%
connect_ml_api/cli.py                  21      0   100%
connect_ml_api/config.py                5      0   100%
connect_ml_api/db.py                   10      0   100%
connect_ml_api/models/__init__.py       0      0   100%
connect_ml_api/models/content.py       47      1    98%
connect_ml_api/routes/__init__.py      11      0   100%
connect_ml_api/routes/content.py       52     25    52%
connect_ml_api/routes/security.py      15      1    93%
connect_ml_api/routes/user.py          52     26    50%
connect_ml_api/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in connect_ml_api/default.toml.
